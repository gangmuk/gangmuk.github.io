...
1. The asyncio event loop submits work to the thread pool
2. The event loop sets up callbacks for when threads complete
3. Threads execute and then need to synchronize with the event loop
4. `asyncio.gather()` must coordinate the completion of multiple threads
...
For ML predictions specifically, batch processing is usually the most efficient:
...
I implemented all three solutions and tested them with batches of 10 inputs:

```
Original (asyncio + to_thread): 6583ms
Solution 1 (Direct ThreadPool): 212ms
Solution 2 (Dedicated Pool): 487ms
Solution 3 (Batch Processing): 43ms
```

The most dramatic improvement came from batch processing, which is ideal for ML prediction workloads. But even the other solutions showed 10-30x performance improvements.
...
3. **Measure everything**: Without detailed timing logs, I might never have discovered that `asyncio.gather()` was the culprit.

4. **Understand the systems you're working with**: A deeper understanding of asyncio and ThreadPoolExecutor would have helped me avoid this issue from the start.

5. **Test alternatives**: Different concurrency approaches can have dramatically different performance characteristics.
...
- [FastAPI Concurrency and Background Tasks](https://fastapi.tiangolo.com/async/)
- [Python `asyncio` documentation](https://docs.python.org/3/library/asyncio.html)
- [Python `concurrent.futures` documentation](https://docs.python.org/3/library/concurrent.futures.html)
- [CPU-bound vs I/O-bound tasks in Python](https://python.land/async-python/cpu-bound-vs-io-bound)
- [The Python GIL and its effects on ThreadPoolExecutor](https://docs.python.org/3/c-api/init.html#thread-state-and-the-global-interpreter-lock)